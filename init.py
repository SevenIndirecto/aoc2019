...
for y in years:
    ...
    for d in (d for d in days if (y < last_advent_of_code_year or d <= last_advent_of_code_day)):
        ...
        if MAKE_URL and (not os.path.exists(day_pos+"/link.url") or OVERWRITE):
            ...
            url.write(link+str(y)+"/day/"+str(d))
            url.close()
